...
- **Done** (`done.wav`, bell arpeggio): Claude finished the turn and nothing is still running.
- **Agents running** (`workflow.wav`, two soft bells): the turn ended but subagents or a workflow are still working in the background. You get **Done** later, when they report back and Claude finishes.
- **Awaiting input** (`needs-input.wav`, rising "huh?"): Claude is actually blocked waiting on you.
...
| `sounds/*.wav` | The three sounds, all synthesized from code (original, free to use). |
| `sounds/make-sounds.py` | Regenerates all three sounds (pure Python, no dependencies). |
...
**Synthesized sounds:** edit `sounds/make-sounds.py`, then run `python3 make-sounds.py` (writes all three into `~/.claude/sounds/`) or `python3 make-sounds.py <folder>` to write somewhere else.
...
MIT. The bundled sounds are generated by `make-sounds.py` and are covered by the same license.
